...
int	M;
		...
		__in >> __expected >> N >> K >> M;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << K << ", " << M << std::endl;
		std::cout.flush();

		__timer_start();

		ColorfulLineGraphs __object;
		int __actual = __object.countWays(N, K, M);
