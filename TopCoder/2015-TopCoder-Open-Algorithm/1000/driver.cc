...
vector <int>	cost;
		...
		__in >> __expected >> parent >> cost;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << parent << ", " << cost << std::endl;
		std::cout.flush();

		__timer_start();

		MinimumCuts __object;
		int __actual = __object.costPaid(parent, cost);
