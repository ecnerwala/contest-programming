...
#include "ApplesAndOrangesEasy.cc"
...
vector <int>	info;
		...
		__in >> __expected >> N >> K >> info;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << K << ", " << info << std::endl;
		std::cout.flush();

		__timer_start();

		ApplesAndOrangesEasy __object;
		int __actual = __object.maximumApples(N, K, info);
