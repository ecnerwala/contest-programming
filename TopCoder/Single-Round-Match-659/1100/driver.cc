...
int	X;
		...
		__in >> __expected >> N >> edges >> X;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << edges << ", " << X << std::endl;
		std::cout.flush();

		__timer_start();

		PublicTransitHard __object;
		int __actual = __object.countValidTeleporters(N, edges, X);
