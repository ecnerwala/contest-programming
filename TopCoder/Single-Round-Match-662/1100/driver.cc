...
#include "MultiplicationTable.cc"
...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> firstRow;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << firstRow << std::endl;
		std::cout.flush();

		__timer_start();

		MultiplicationTable __object;
		vector <int> __actual = __object.getMultiplicationTable(firstRow);
