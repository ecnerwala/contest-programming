...
int	K;
		...
		__in >> __expected >> R >> C >> K;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << R << ", " << C << ", " << K << std::endl;
		std::cout.flush();

		__timer_start();

		PaintTheRoom __object;
		string __actual = __object.canPaintEvenly(R, C, K);
