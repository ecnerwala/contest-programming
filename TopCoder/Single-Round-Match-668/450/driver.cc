...
vector <int>	to;
		...
		__in >> __expected >> N >> from >> to;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << N << ", " << from << ", " << to << std::endl;
		std::cout.flush();

		__timer_start();

		WalkingToSchool __object;
		string __actual = __object.canWalkExactly(N, from, to);
