...
vector <int>	m;
		...
		__in >> __expected >> n >> h >> t >> val0 >> a >> b >> m;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << h << ", " << t << ", " << val0 << ", " << a << ", " << b << ", " << m << std::endl;
		std::cout.flush();

		__timer_start();

		SettingShield __object;
		long long __actual = __object.getOptimalCost(n, h, t, val0, a, b, m);
