...
int	s2;
		...
		__in >> __expected >> a >> b >> s1 >> s2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << a << ", " << b << ", " << s1 << ", " << s2 << std::endl;
		std::cout.flush();

		__timer_start();

		TwoEntrances __object;
		int __actual = __object.count(a, b, s1, s2);
