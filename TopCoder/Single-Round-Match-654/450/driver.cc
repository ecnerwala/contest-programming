...
#include "SuccessiveSubtraction.cc"
...
vector <int>	v;
		...
		__in >> __expected >> a >> p >> v;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << a << ", " << p << ", " << v << std::endl;
		std::cout.flush();

		__timer_start();

		SuccessiveSubtraction2 __object;
		vector <int> __actual = __object.calc(a, p, v);
